...
import com.matthew.cn.newsreading.api.GankService;
import com.matthew.cn.newsreading.api.NewsListService;
...
public class RetrofitManager {
    private NewsListService mNewsService;
    ...
    public RetrofitManager(@HostType.HostTypeChecker int hostType) {
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .build();
        mNewsService = retrofit.create(NewsListService.class);
        mGankService = retrofit.create(GankService.class);
    }

    /**
     * @param hostType NETEASE_NEWS_VIDEO：1 （新闻，视频），GANK_GIRL_PHOTO：2（图片）;
     */
    ...
    private OkHttpClient getOkHttpClient() {
        if (sOkHttpClient == null) {
            synchronized (RetrofitManager.class) {
                Cache cache = new Cache(new File(NewsApplication.getAppContext().getCacheDir(), "HttpCache"),
                        1024 * 1024 * 100);
                if (sOkHttpClient == null) {
